...
use Swag\MigrationOxid\Profile\Oxid\DataSelection\DefaultEntities as DefaultEntitiesAlias;
use SwagMigrationAssistant\Migration\MigrationContextInterface;
use SwagMigrationAssistant\Migration\TotalStruct;

abstract class ProductMultiSelectTextPropertyRelationReader extends AbstractReader
{
    /**
     * @var int
     */
    ...
    public function readTotal(MigrationContextInterface $migrationContext): ?TotalStruct
    {
        ...
        $query = <<<SQL
SELECT COUNT(entity_text.entity_id)
FROM {$this->tablePrefix}catalog_product_entity_text AS entity_text
WHERE entity_text.value IS NOT NULL
AND entity_text.entity_type_id = {$this->productEntityTypeId}
AND entity_text.store_id = 0
AND entity_text.attribute_id IN (
...
SQL;
        ...
        return new TotalStruct(DefaultEntitiesAlias::PRODUCT_MULTI_SELECT_TEXT_PROPERTY, $total);
    }
...
INNER JOIN {$this->tablePrefix}catalog_product_entity_text AS entity_text ON  entity_text.entity_id = product.entity_id
INNER JOIN {$this->tablePrefix}eav_attribute AS eav ON eav.attribute_id = entity_text.attribute_id AND eav.is_user_defined = 1
